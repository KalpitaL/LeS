...
namespace UnionMarine_Http_Routine
{
    ...
    {
        public int iRetry = 0, nStatus_FilterIndx=0;
        ...
        public string currentDate = "", SITEURL = "", cQuote_filepath = "",cQuote_Attachpath="", ProcessorName = "", sDoneFile = "", sAuditMesage = "", cUploadAttachment="",
            cTemp_PDFPath="",cErrorPath="", cCookieVal = "", ModuleName="",cQte_href="",cBackup_Path="",cProcessQuote="",cCheck_Partdet="";
        public double dGrandTotal, dFile_GrandTotal,dBuyerTotal;
        public string[] BuyerNames, Buyer_Supplier_LinkID, Actions, OnlySubmitSupp, SuppCodes, Login_Credentials,SiteUrl;
        ...
        public void GetAppConfigSettings()
        {
            try
            {
                ...
                SiteUrl = Convert.ToString(ConfigurationManager.AppSettings["SITE_URL"]).Split('|');
               ...
               // SITEURL = URL = Convert.ToString(ConfigurationManager.AppSettings["SITE_URL"]);
                ...
                cProcessQuote = Convert.ToString(ConfigurationManager.AppSettings["PROCESS_QUOTE"]);
                ...
                cCheck_Partdet = Convert.ToString(ConfigurationManager.AppSettings["CHECK_PARTDETAILS"]);
                ...
                AuditPath = (Convert.ToString(ConfigurationManager.AppSettings["AUDIT_PATH"]) != "") ? Convert.ToString(ConfigurationManager.AppSettings["AUDIT_PATH"]) : AppDomain.CurrentDomain.BaseDirectory + "Audit";
                LogPath = (Convert.ToString(ConfigurationManager.AppSettings["LOG_PATH"]) != "") ? Convert.ToString(ConfigurationManager.AppSettings["LOG_PATH"]) : AppDomain.CurrentDomain.BaseDirectory + "Log";
            ...
            {
                LogText = sAuditMesage = "Exception during initialise: " + e.GetBaseException().ToString();
             //   SetAuditLogFile("", "", ModuleName, "", "", "Error", sAuditMesage);
            }
        }

        public void Download_FileProcess()
        {
            if (!string.IsNullOrEmpty(cProcessQuote) && cProcessQuote.ToUpper() == "TRUE")
            {
                ProcessQuote();
            }
        }
        ...
        private bool LoginProcess()
        {
            try
            {
                if (SiteUrl.Length > 0)
                {
                    for (int k = 0; k < SiteUrl.Length; k++)
                    {
                        URL = SiteUrl[k]; bool IsResult = false;
                        try
                        {
                            IsResult = LoadURL("", "", "", true);
                        }
                        ...
                        if (IsResult)
                        {
                            ...
                            else
                            {
                                ...
                                LogText = sAuditMesage = "Unable to login Union Marine Site,Login failed.";
                                SetAuditLogFile(BuyerCode, SupplierCode, ModuleName, "", "", "Error", "LeS-1014:Unable to login");
                            }
                            break;
                        }
                    }
            ...
            {
                LogText = sAuditMesage = "Exception while login : " + ex.GetBaseException().ToString();
                SetAuditLogFile(BuyerCode, SupplierCode, ModuleName, "", "", "Error", "LeS-1014:Unable to login due to "+ ex.GetBaseException().ToString());
            }
            return IsLoggedin;
        }
        ...
        public bool CheckLogin()
        {
            ...
            try
            {
                ...
                {
                    string[] cDet = Login_Credentials[0].Split('='); SupplierCode = cDet[0];
        ...
        public void ProcessQuote()
        {
            if (!string.IsNullOrEmpty(cQuote_filepath))
            {
                    ...
                    if (quoteFiles.Length > 0)
                    {
                        if (LoginProcess())
                        {
                            foreach (FileInfo fQuote in quoteFiles)
                            {
                                ...
                                _interchange = _mtmlClass.Load(fQuote.FullName); IsDeclined = _interchange.DocumentHeader.IsDeclined;
                                string SupplierCode = _interchange.Sender; string BuyerLinkCode = _interchange.Recipient; string cF_MessageNumber = _interchange.DocumentHeader.MessageNumber;
                                LogText = "Processing Quote File " + Path.GetFileName(fQuote.FullName) + " started.";
                             
                                bool Is_Search = GetSearchFilterList(_htmlInvDoc, cF_MessageNumber);
                                if (Is_Search) { GetFilterRecords(_htmlInvDoc, _interchange, fQuote.FullName, SupplierCode, cF_MessageNumber); }
                                LogText = "Processing Quote File ended.";
                                LogText = "---------------------------------------";
                            }
                        }
                    }
                ...
                else { LogText = "Invalid Quote File Path"; }
            }
        }

        public bool GetSearchFilterList(HtmlDocument _htmlInvDoc, string cF_MessageNumber)
        {
            bool result = false;
            try
            {
                ...
                HtmlNode _sRFQstatus = _htmlInvDoc.DocumentNode.SelectSingleNode("//select[@id='SearchRfqStatus']/option[@selected]");
                ...
                _htmlInvDoc.GetElementbyId("SearchKeyword").SetAttributeValue("value", cF_MessageNumber);

                string _searchType = (_sType != null) ? _sType.OuterHtml : "rfq";
                ...
                string _searchRFQstatus = (_sPOstatus != null) ? _sPOstatus.OuterHtml :"RFQ Number";
                ...
                string _searchKeyword = _htmlInvDoc.GetElementbyId("SearchKeyword").InnerText; _searchKeyword = cF_MessageNumber;
                this.URL = SITEURL + "invoice";
                _httpWrapper._SetRequestHeaders[HttpRequestHeader.Cookie] = "PHPSESSID=" + _httpWrapper._dctSetCookie["PHPSESSID"];
                Post_Data = @"data%5BSearch%5D%5BSearchFor%5D=" + _searchType +// "&data%5BSearch%5D%5BNumber%5D=" + _searchNo +
                   "&data%5BSearch%5D%5BInvoiceStatus%5D=" + _searchInvstatus + "&data%5BSearch%5D%5BPoStatus%5D=" + _searchPOstatus + "&data%5BSearch%5D%5BRfqStatus%5D=" + _searchRFQstatus + "&data%5BSearch%5D%5BPoStatus1%5D=" + _searchPOstatus1 +
        ...
        public void GetFilterRecords(HtmlDocument _htmlDoc,MTMLInterchange _interchange,string cXML_FileName,string cSuppCode,string cF_MessageNumber)
        {
            ...
            string cVrNo = "", cHREF = ""; SupplierCode = cSuppCode; int nVrNo_notPresent = 0;
            try
            {
                HtmlNodeCollection _collectTabData = _htmlDoc.DocumentNode.SelectNodes("//div[@id='tab_requisition']//table/tbody/tr");
                if (_collectTabData != null && _collectTabData.Count == 1)//>0
                {
                    for (int k = 0; k < _collectTabData.Count; k++)
                    {
                        HtmlNodeCollection _arr = _collectTabData[k].ChildNodes;
                        if (_arr.Count > 0)
                        {
                            for (int j = 0; j < _arr.Count; j++)
                            {
                                if (_arr[j].NextSibling != null && _arr[j].NextSibling.Name != "#text")
                                {
                                    LogText = "Quote update -" + (j + 1);

                                    string ctext = _arr[j].NextSibling.InnerText;
                                    ...
                                    if (aTags != null)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (_interchange.DocumentHeader.MessageNumber == cVrNo)
                                            {
                                                string cdet = (attrtitle != null) ? attrtitle.Value : " ";
                                                if (cdet.ToUpper() == "SUBMIT QUOTE")
                                                {
                                                    slQteheader.Clear();
                                                    if (!File.Exists(cBackup_Path + @"\" + Path.GetFileName(cXML_FileName)))
                                                    {
                                                ...
                                                {
                                                    if (string.IsNullOrEmpty(cdet.Trim()) || cdet.ToUpper() == "UPDATE QUOTE")
                                                    {
                                                        LogText = sAuditMesage = "Quote is already updated for VRNO " + cVrNo + ".";//Audit message added 12/04/2019
                ...
                {
                    LogText = sAuditMesage = "RFQ No.: " + cF_MessageNumber + " not present in the list";
                    MoveFiles(cXML_FileName, cBackup_Path + @"\" + Path.GetFileName(cXML_FileName));
                }
            }
            catch (Exception ex)
            {
                LogText = "Error while Processing file : " + ex.GetBaseException().ToString();
                ...
                SetAuditLogFile(BuyerCode, SupplierCode, ModuleName, Path.GetFileName(cXML_FileName), cVrNo, "Error", "LeS-1004:Unable to process file due to  " + ex.GetBaseException().ToString());
            }
        }
        ...
        {
            bool _result = false,IsTotalMatch=false; HtmlDocument _htmlQuotDoc = null; string cAttach_File = "";
            ...
            try
            {
                if (!string.IsNullOrEmpty(cUploadAttachment) && cUploadAttachment.ToUpper() == "TRUE")
                {
                   ...
                   // AttachDetails(cAttach_File);
                    ReadAttachments(cXML_FileName);//added by Kalpita on 30/12/2019
                }
                    ...
                    if (PageHeader.ToUpper().Contains("QUOTE"))
                    {
                        ...
                        if (_saveNode != null)
                        {
                            ...
                            if (IsQte_Save)
                            {
                                ...
                                {
                                    LogText = "Quotation for VRNO " + cVrNo + " saved successfully.";//added by Kalpita on 02/04/2020
                                    ...
                                    if (dGrandTotal > 0)
                                    {
                                        if (Convert.ToInt32(dGrandTotal) == Convert.ToInt32(dFile_GrandTotal)) { IsTotalMatch = true; }//changed by Kalpita on 27/12/2019,24/04/2020
                                        else if (Convert.ToInt32(dGrandTotal) == Convert.ToInt32(dBuyerTotal)) { IsTotalMatch = true; }//changed by Kalpita on 27/12/2019
                                        if (IsTotalMatch)
                                        {
                                            ...
                                            LogText = "Quotation for VRNO " + cVrNo + " updated successfully.";
                                            //this.URL = SITEURL + "invoice"; ; Post_Data = "";
                                            //_httpWrapper._SetRequestHeaders[HttpRequestHeader.Cookie] = "PHPSESSID=" + _httpWrapper._dctSetCookie["PHPSESSID"];
                                            //_result = LoadURL("", "", ""); _htmlInvDoc = _httpWrapper._CurrentDocument;
                                            MoveFiles(cXML_FileName, cBackup_Path + @"\" + Path.GetFileName(cXML_FileName));
                                        }
                                        else
                                        {
                                            LogText = sAuditMesage = "Total amount mismatch for file -" + cFile;
                                            SetAuditLogFile(BuyerCode, cSuppCode, ModuleName, cFile, cVrNo, "Error", "LeS-1008.1:Unable to save quote due to amount mismatch");
                                            MoveFiles(cXML_FileName, cErrorPath + @"\" + Path.GetFileName(cXML_FileName));
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            string cDecline = (IsDeclined) ? " and is declined." : " and updated";
                                            LogText = sAuditMesage = "Quotation for VRNO " + cVrNo + " has zero Total " + cDecline;
                                            MoveFiles(cXML_FileName, cBackup_Path + @"\" + Path.GetFileName(cXML_FileName));
                                        }
                                    }
                                }
            ...
            catch (Exception e)
            {
                ...
                SetAuditLogFile(BuyerCode, cSuppCode, ModuleName, cFile, cVrNo, "Error", "LeS-1008:Unable to save quote due to"+ e.GetBaseException().ToString());
                MoveFiles(cXML_FileName, cErrorPath + @"\" + Path.GetFileName(cXML_FileName));
            }
            return _result;
        }
        ...
        private void ProcessHeader_ItemDetails(HtmlDocument _htmlQuotDoc, MTMLInterchange _interchange, string cVrNo)
        {
            ...
            double fGrndTotal = 0.0d,fOtherCharges=0.0d,fBuyerItemTotal=0.0d;
            ...
            try
            {
               ...
               _SuppCurr.InnerHtml = cCurrency; slQteheader.Add("CURRENCY", cCurrency);
                ...
                if (_interchange != null)
                {
                    ...
                    foreach (MonetaryAmount _amt in _interchange.DocumentHeader.MonetoryAmounts)
                    {
                        switch (_amt.Qualifier)
                        {
                            ...
                            case MonetoryAmountQualifier.GrandTotal_259: cGrandTotal = _amt.Value.ToString("0.00"); slQteheader.Add("GRAND_TOTAL", cGrandTotal);
                                ...
                                break;
                            case MonetoryAmountQualifier.Deposit_97: fGrndTotal += _amt.Value; fOtherCharges += _amt.Value; //added on 02/05/2019 include deposit charges (WSS)
                                break;
                            case MonetoryAmountQualifier.BuyerItemTotal_90:  fBuyerItemTotal = _amt.Value; //added on 27/12/2019 include Buyer Item total
                                break;
                        }
                    }
                    ...
                    dBuyerTotal = Math.Round(fOtherCharges + fBuyerItemTotal, 2, MidpointRounding.AwayFromZero);//added by Kalpita on 27/12/2019
                }
            }
            catch (Exception ex)
            {
                ...
                foreach (LineItem _itm in _itmCollect)
                {
                    for (int i = 0; i < parsedTblBody.Count; i++)
                    {
                        ...
                        if (sldet.Count > 2 && !sldet[0].Contains("Total"))
                        {
                            ...
                            if (cFItemno == cItemNo)
                            {
                                if (!string.IsNullOrEmpty(cCheck_Partdet) && cCheck_Partdet.ToUpper() == "TRUE")
                                {
                                    ...
                                    Is_Partdet = (IS_PartDesc_same || Is_PartCode_same);
                                }
                                else { Is_Partdet = false; }
                                if (!Is_Partdet)
                                {
                                    id = Convert.ToInt32(_Rmknode.GetAttributeValue("id", "").Replace("QuoteRemark", ""));
                                    if (cQte_UOM.ToUpper() != cFUOM.ToUpper()) { cFRemarks += Environment.NewLine + " UOM :" + cFUOM; }
        ...
        //added by Kalpita on 30/12/2019
        ...
        private string AttachDetails(string cAttachFile)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(cAttachFile))
                {
...
//        }
        ...
        //private bool Check_RFQExists(HtmlNode _collectTabData)
        //{
        //    bool IsExists = false;
        //    HtmlNodeCollection _collectData = _collectTabData.SelectNodes("//table[contains(@class, 'table table-bordered table-hover')]/thead/tr");
        //    if (_collectData != null && _collectData.Count > 0)
        //    {
        //        for (int i = 0; i < _collectData.Count; i++)
        //        {
        //            HtmlNodeCollection _arr = _collectData[i].ChildNodes;
        //            {
        //                for (int j = 0; j < _arr.Count; j++)
        //                {
        //                    if (_arr[j].NextSibling != null)
        //                    {
        //                        string aTags = _arr[j].NextSibling.InnerText.ToUpper();

        //                        if (aTags != null)
        //                        {
        //                            if (aTags.Contains("RFQ")) { IsExists = true; break; }
        //                            //if (aTags.Contains("STATUS")) { nStatus_FilterIndx = j; }
        //                        }
        //                    }
        //                }
        //            }
        ...
        //public void GetFilterRecords1(HtmlDocument _htmlDoc,MTMLInterchange _interchange,string cXML_FileName,string cSuppCode,string cF_MessageNumber)
        //{
        //    List<string> slHeader = new List<string>(); slHeader.Clear(); string cFileName = "";
        //    Dictionary<int, string> slRecords = new Dictionary<int, string>(); slRecords.Clear();
        //    string cVrNo = "", cHREF = ""; SupplierCode = cSuppCode; int nVrNo_notPresent = 0;
        ...
        //            for (int k = 0; k < _collectTabData.Count; k++)
        //            {
        //                bool IsRfqPresent = Check_RFQExists(_collectTabData[k]);
        //                if (IsRfqPresent)
        //                {
        ...
        //                else
        //                {
        //                    LogText = "Reference No.: " + cF_MessageNumber + " not present in the list";
        //                    MoveFiles(cXML_FileName, cBackup_Path + @"\" + Path.GetFileName(cXML_FileName));
        //                }
        //            }
        //        }
