...
var New_BuyerSupplier_Wizard = function () {

    var handleNew_BuyerSupplier_Wizard = function () {
        ...
        SetCommondetails(); SetDetails(_ADDRTYPE); setupBXLSMappingTableHeader(); setupBPDFMappingTableHeader(); $('#btnclose').addClass('closebtn');
        $('.form-wizard').bootstrapWizard({
            ...
            onInit: function (tab, navigation, index) {
                if (_ADDRTYPE.toUpperCase() == 'SUPPLIER') {
                    $('#lstcopy').hide(); $('#lstsync').hide();
                    var _wizspn = $('li a span.number').length; $('li a span.number')[_wizspn - 1].outerHTML = '<span class="number"> ' + (_wizspn - 1) + ' </span>';
                }
            },
            ...
            onNext: function (tab, navigation, index) {
                ...
                if (index == 2) {
                    var _isDefsetvalid = ValidateDefaultSettings(); if (_isDefsetvalid) {
                        if (_ADDRTYPE.toUpperCase() == 'BUYER') { $('#btnNext').html(_savecontinuetxt); }
                        var _selectdefval = $('#cbDefFormat option:selected').val(); SetDefaultRules(_ADDRTYPE, _selectdefval); result = true;
                    } else { result = false; }
                }
                if (index == 3) {
                    var _isRulevalid = ValidateDefaultRules(); if (_isRulevalid) {
                        var _buyres = SavePartyDetails(_ADDRTYPE, _scheckid); if (_buyres != '') { result = true; }
                    } else { result = false; }
                }
                if (index == 4) { InitializeMappings(); CopyMappings(); GetServerDetails(); result = true; }
        ...
        $('#cbDefFormat').live("change", function (e) {
            var _selectdefval = $('#cbDefFormat option:selected').val(); var _sldet = GetDocFormatDetail(_selectdefval); var _selecteddeftext = $('#cbDefFormat option:selected').text().trim();
        ...
        $('#dataGridNewDRule').on('change', 'tr', function (e) {
        ...
        function ValidateDefaultRules() {
            ...
            $('input[type=checkbox]:checked').each(function () {
                var tr = $(this).closest('tr'); if (oNDRTable.fnGetData(tr) != null) {
                    ...
                    var _ruleval = $(_id).val(); var _rulecd = oNDRTable.fnGetData(tr)[3]; var _ruleid = oNDRTable.fnGetData(tr)[7];
                    var _ruledet = tr[0]._DT_RowIndex + '#RULEID|' + +Str(_ruleid) + '#' + 'RULE_CODE|' + Str(_rulecd) + '#' + 'RULE_VALUE|' + Str(_ruleval);
                    if (Str(_ruleval) != '') { if (Str(_ruleval).toUpperCase() != 'NOT SET') { _scheckid.push(Str(_ruledet)); } else { _isvalid = false; } } else { _isvalid = false; }
                }
            });
            return _isvalid;
        };
        ...
        $('#btnAddFormat').live('click', function (e) {
            e.preventDefault(); var _lstdeffrmt = []; _lstdeffrmt.push('DOCFORMATID|0');
            _lstdeffrmt.push('ADDR_TYPE|' + _ADDRTYPE); _lstdeffrmt.push('DOCUMENT_FORMAT|' + Str($('#txtDefFormat').val()));
            _lstdeffrmt.push('IMPORT_PATH|' + Str($('#txtDFImportPath').val())); _lstdeffrmt.push('EXPORT_PATH|' + Str($('#txtDFExportPath').val()));
            var _isvalid = Validate_NDefFormat(); if (_isvalid) { SaveDocumentFormat(_lstdeffrmt); $("#ModalNewFormat").modal('hide'); }
        });
        ...
        function CopyMappings() {
            ...
            for (var l = 0; l < xlsrows.length; l++) {
                ...
                for (var k = 0; k < _lstSelectedxlRowindx.length; k++) {
                    if (_lstSelectedxlRowindx[k] == l) {_cpyPMapping = [];
            ...
            for (var l = 0; l < pdfrows.length; l++) {
                ...
                for (var k = 0; k < _lstSelectedxlRowindx.length; k++) {
                    if (_lstSelectedxlRowindx[k] == l) { _cpyPMapping = [];
                        ...
                        _cpyPMapping.push('PDFMAPID|' + _pdfmapid); _cpyPMapping.push('BUYERID|' + ID); _cpyPMapping.push('DOCTYPE|' + _doctype); _cpyPMapping.push('FORMAT_MAPCODE|' + _fmtmapcode);
        ...
        $('#btnConfrmCancel').live('click', function (e) { e.preventDefault(); top.window.close(); parent.window.close(); });
        ...
        $("#txtName").autocomplete({
            source: function (request, response) {
                $.ajax({
                    type: "POST",
                    async: false,
                    url: "NewBuyerSupplierWizard.aspx/GetAddressName_Search",
                    data: "{'SEARCHTEXT':'" + $("#txtName").val() + "','ADDRTYPE':'"+_ADDRTYPE+"'}",
    ...
    function SetCommondetails() {
        ...
        $.ajax({
            type: "POST",
            async: false,
            url: "NewBuyerSupplierWizard.aspx/GetConfigDetails",
            data: '{}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
    ...
    $('input[type="radio"]').on('click', function () {
        InitializeMappings(); $('#tblXls').show(); $('#tblPdf').hide(); $('#tblXls').hide();
        _ExistBYRID = $('#cbBuyers option:selected').val();
        if ($(this).attr('id') == 'rdExcel') { $('#tblXls').show(); GetBuyerExcelMapp(_ExistBYRID); }
        else if ($(this).attr('id') == 'rdPDF') { $('#tblXls').hide(); $('#tblPdf').show(); GetBuyerPDFMapp(_ExistBYRID); }
    });
    /*Config Settings*/
    ...
    function fnBConfgSettings(_ADDRTYPE) {
        var sOut = ''; var _str = ''; var _cnfgMailsubj = ''; var _chklst = '';
        ...
        var sOut = '<div class="row">' +
                ...
                ' <div class="col-md-2" style="text-align:right;margin-top:5px;"><label class="dvLabel"> Document Format <span class="required" aria-required="true"> * </span> </label> </div>' +
    ...
    function FillGroupFormat(ADDRTYPE) {
        ...
        $.ajax({
            type: "POST",
            async: false,
            url: "NewBuyerSupplierWizard.aspx/GetDocumentFormat",
                ...
                try {
                    ...
                    if (Dataset.NewDataSet != null) {
                        ...
                        if (Table != undefined && Table != null) {
                            if (Table.length != undefined) { for (var i = 0; i < Table.length; i++) { _lstgrpfrmt.push(Str(Table[i].ID) + "|" + Str(Table[i].FORMAT)); }  }
                            else {  if (Table.FORMAT != undefined){  _lstgrpfrmt.push(Str(Table.ID) + "|" + Str(Table.FORMAT));  } }
                        }
                    }
                }
    ...
    var GetDocFormatDetail = function (ID) {
        $.ajax({
            type: "POST",
            async: false,
            url: "NewBuyerSupplierWizard.aspx/GetDocumentFormatDetails",
            data: "{'ID':'" + ID + "'}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                try {    _sldfdet = JSON.parse(response.d);  }
    ...
    function ValidateDefaultSettings() { var _valid = true; var _deffrmt = $('#cbDefFormat option:selected').val(); if (_deffrmt == '') { $('#txtImpPath').addClass('error'); $('#txtExpPath').addClass('error'); _valid = false; } return _valid; };
    ...
    function fnDefaultFormatDetails() {
        ...
        var sOut = '<div class="row"> <div class="col-md-12"><div class="row"><div class="col-md-12"><div class="form-group">' +
              ' <div class="col-md-4" style="text-align:right;margin-top:5px;"><label class="dvLabel"> Document Format <span class="required" aria-required="true"> * </span> </label> </div>' +
              ' <div  class="col-md-8"><input type="text" class="form-control" id="' + _codeid + '" value="' + _code + '"/></div>' +
              ' </div></div></div><div class="row" style="margin-top:3px;"><div class="col-md-12"><div class="form-group">' +
              ' <div class="col-md-4" style="text-align:right;margin-top:5px;"><label class="dvLabel"> Import Path </label> </div>' +
              ' <div  class="col-md-8"><input type="text" class="form-control" id="' + _imppathid + '" value="' + _ImpPath + '"/></div>' +
              ' </div></div></div><div class="row" style="margin-top:3px;"><div class="col-md-12"><div class="form-group">' +
              ' <div class="col-md-4" style="text-align:right;margin-top:5px;"><label class="dvLabel"> Export Path </label> </div>' +
              ' <div  class="col-md-8"><input type="text" class="form-control" id="' + _exppathid + '" value="' + _ExpPath + '"/></div>' +
              ' </div></div></div></div></div>';
        return sOut;
    };

    function SaveDocumentFormat(_nfieldval) {
        ...
        $.ajax({
            type: "POST",
            async: false,
            url: "NewBuyerSupplierWizard.aspx/SaveDocumentFormat",
            data: data2send,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                try {var res = Str(response.d); RepopulateDcoumentFormat(_ADDRTYPE); Metronic.unblockUI(); }
                catch (err) {  toastr.error("Lighthouse eSolutions Pte. Ltd.", 'Unable to update Document Format:' + err); Metronic.unblockUI(); }
            },
    ...
    function RepopulateDcoumentFormat(ADDRTYPE) {
    ...
    var CheckExistingFormat = function (FORMAT) {
        ...
        $.ajax({
            type: "POST",
            async: false,
            url: 'NewBuyerSupplierWizard.aspx/CheckExistingFormat',
            data: "{ 'FORMAT':'" + FORMAT + "'}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
    ...
    function fnheaderFormatDetails(ADDRTYPE) {
        ...
        var tid = "Table"; var _tbodyid = "tblBody";  var _codeid = 'txtCode'; var _bnameid = 'txtName'; var _contactPersid = 'txtContactPerson'; var _emailid = 'txtEmail';
        var _countryid = 'txtCountry'; var _weblnkid = 'txtWebLink'; var _downloadpathid = 'txtDownloadPath'; var _uploadpathid = 'txtUploadPath';
        var _cityid = 'txtCity'; var _postalid = 'txtPostalCode'; var _txtdisabled = 'disabled';  _code = GetNextCode(ADDRTYPE);
    ...
    var GetNextCode = function (ADDRTYPE) {
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function (response) {
                try {   var _lstres = Str(response.d).split('||'); res = _lstres[0]; _uplDwlPath = _lstres[1].trim(); _configpath = _lstres[2].trim();}
                catch (err) {  toastr.error("Lighthouse eSolutions Pte. Ltd.", 'Get Next Code :' + err); }
            },
            failure: function (response) {  toastr.error("failure get", response.d);},
            error: function (response) {  toastr.error("error get", response.responseText);}
        });
    ...
    function ValidateParty(id) {
        ...
        if (_code == '') { $('#txtCode').addClass('error'); _valid = false; }
            ...
            var isexist = CheckExisting(_ADDRTYPE, _code, id);
    ...
    var CheckExisting = function (ADDRTYPE, ADDR_CODE, ID) {
        ...
        $.ajax({
            ...
            success: function (response) {
                try { res = Str(response.d); }
                catch (err) {   toastr.error("Lighthouse eSolutions Pte. Ltd.", 'cannot validate new :' + ADDRTYPE + err); }
            },
            failure: function (response) {   toastr.error("failure get", response.d); },
            error: function (response) { toastr.error("error get", response.responseText); }
        });
    ...
    function SaveDetails(ADDRTYPE, _nfieldval, _nfieldval1) {
        ...
        $.ajax({
            type: "POST",
            async: false,
            url: "NewBuyerSupplierWizard.aspx/SaveDetails",
            data: data2send,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                try {   ID = Str(response.d); Metronic.unblockUI(); }
                catch (err) { toastr.error("Lighthouse eSolutions Pte. Ltd.", 'Unable to update :' + ADDRTYPE + err); Metronic.unblockUI();  }
            },
            failure: function (response) { toastr.error("failure get", response.d); Metronic.unblockUI();},
            error: function (response) { toastr.error("error get", response.responseText); Metronic.unblockUI();  }
        });
    ...
    function InitializeDefaultRules()
    {
        ...
        oNDRTable = table3.dataTable({
           ...
           { 'targets': [6], width: '30px' }, { 'targets': [7], visible: false }
            ],
          ...
          //  "scrollY": "300px",
            ...
            "fnRowCallback": function (nRow, aData, iDisplayIndex) {
                ...
                $('td:eq(0)', nRow).html(_chkdiv); var _id = 'cbRuleValue' + nRow._DT_RowIndex; var _defruleval = (Str(aData[6]) == '') ? 'NOT SET' : Str(aData[6]);
    ...
    function SetDefaultRules(_ADDRTYPE, _selectdefval) {  InitializeDefaultRules();
        var FORMAT = GetFormat(ADDRESSID, _ADDRTYPE); var _isrecpres = GetDefaultFormat_RulesGrid(_selectdefval); if (_isrecpres == '0') { GetRulesGrid(ADDRESSID, FORMAT); }
    };
        ...
        try {
            ...
            if (Table.length != undefined && Table.length != null) {
                ...
                for (var i = 0; i < Table.length; i++) {
                    ...
                    cells[4] = Str(Table[i].RULE_DESC);  cells[5] = _arrcommt; cells[6] = Str(Table[i].RULE_VALUE); cells[7] = Str(Table[i].RULEID);
                ...
                if (Table.RULEID != undefined && Table.RULEID != null) {
                    var t = $('#dataGridNewDRule').dataTable(); var r = jQuery('<tr id=' + 1 + '>');
                    ...
                    cells[4] = Str(Table.RULE_DESC);cells[5] = _arrcommt;  cells[6] = Str(Table.RULE_VALUE); cells[7] = Str(Table.RULEID);
                    var ai = t.fnAddData(cells, false);
                    t.fnDraw();
                }
            }
    ...
    var GetDefaultFormat_RulesGrid = function (DOCFORMATID) {
        var _isrecordspresent = '0';
        Metronic.blockUI('#portlet_body');
        $.ajax({
            type: "POST",
            async: false,
            ...
            success: function (response) {
                try {
                    if (response.d != '') {
                        var DataSet = JSON.parse(response.d);
                        if (DataSet.NewDataSet != null) {
                            Table = DataSet.NewDataSet.Table; FillRulesGrid(Table); _isrecordspresent = '1';
                        }
                        else $('#dataGridNewDRule').DataTable().clear().draw();
                    }
                    Metronic.unblockUI();
    ...
    function SavePartyDetails(ADDRTYPE, _scheckid) {
        ...
        var _city = $('#txtCity').val(); var _postal = $('#txtPostalCode').val(); var _islesconnect = ($('#chkLesConnect').is(':checked')) ? 1 : 0;
        var _imp = $('#txtImpPath').val(); var _exp = $('#txtExpPath').val(); var _map = $('#txtMapping').val(); var _defFormat = $('#cbDefFormat option:selected').text(); var _ccemail = $('#txtCCEmail').val();
        ...
        _lstdet.push("DEFAULT_PRICE" + "|" + Str(_dprice)); _lstdet.push("UPLOAD_FILE_TYPE" + "|" + Str(_ufilety)); _lstdet.push("WEB_SERVICE_URL" + "|" + Str(_wurl)); _lstdet.push("ISLESCONNECT" + "|" + Str(_islesconnect));
        if (_ADDRTYPE.toUpperCase() == 'BUYER') {
                    ...
                    if (Dataset.NewDataSet != null) {
                        ...
                        if (_lstEbyrs != undefined && Table != null) {
                            if (Table.length != undefined) {
                                for (var i = 0; i < Table.length; i++) {  _lstEbyrs.push(Str(Table[i].ADDRESSID) + "|" + Str(Table[i].ADDR_NAME)); }  }
                            else {
                                if (Table.FORMAT != undefined) { _lstEbyrs.push(Str(Table.ADDRESSID) + "|" + Str(Table.ADDR_NAME));  }
                            }
                        }
                    }
    ...
    function GetBuyerExcelMapp(_ExistBYRID) {
        if (_ExistBYRID != undefined) {
            var DataSet = GetMappingXLSGrid(_ExistBYRID, 'WIZ'); if (DataSet.NewDataSet != null) { var Table = DataSet.NewDataSet.Table; FillBMappingXLSGrid(Table); SetCheckboxState('dataGridBXlsMapp'); }
            else { $('#chkHdexcel').prop('checked', false);$('#dataGridBXlsMapp').DataTable().clear().draw();} 
        }
    };
    ...
    function GetBuyerPDFMapp(_ExistBYRID) {
        if (_ExistBYRID != undefined) {
            ...
            else { $('#chkHdpdf').prop('checked', false); $('#dataGridBPdfMapp').DataTable().clear().draw();}
        }
    };
    ...
    function SaveExcelMappings(_nfieldval) {
        ...
        $.ajax({
            type: "POST",
            async: false,
            url: "NewBuyerSupplierWizard.aspx/SaveExcelMappings",
            data: data2send,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                try { var res = Str(response.d); Metronic.unblockUI(); }
                catch (err) {toastr.error("Lighthouse eSolutions Pte. Ltd.", 'Unable to copy mapping :' + err); Metronic.unblockUI();  }
            },
            failure: function (response) {  toastr.error("failure get", response.d); Metronic.unblockUI();  },
            error: function (response) { toastr.error("error get", response.responseText); Metronic.unblockUI(); }
        });
    ...
    function SavePdfMappings(_nfieldval) {
        ...
        $.ajax({
            type: "POST",
            async: false,
            url: "NewBuyerSupplierWizard.aspx/SavePDFMappings",
            data: data2send,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
    ...
    function GetSelectedCheckbox(grid) {
        if (grid == 'dataGridBXlsMapp') {
            ...
            for (var l = 0; l < xlsrows.length; l++) {    var tr = $(xlsrows[l]); if ($('input[type="checkbox"]:checked', tr).length > 0) { _lstSelectedxlRowindx.push(l);}  }
        }
        else if (grid == 'dataGridBPdfMapp') {
    ...
    function GetServerSyncConfirmation(_lstselectserv) {
        var _det = ''; var _downloadpathid = ''; var _downloadpath = $('#txtDownloadPath').val(); var _uploadpathid = '';
        ...
        var _hddet = '<div class="row"><div class="col-md-12">' +
            '<div style="padding-left:5px;"><h5 class="alert alert-info">Buyer with path details will be synched with the selected servers. Please Confirm would you like to proceed ?</h5></div>' +
    ...
    function SyncBuyer(ID, _nfieldval, _nfieldval1) {
        ...
        var data2send = JSON.stringify({ ID: ID, slServdet: slServdet, slServpaths: slServpaths,cInsertdet:'1' });
        Metronic.blockUI('#portlet_body');
        setTimeout(function () {
            $.ajax({
                type: "POST",
    ...
    function SaveServer(_nfieldval, callback) {
        ...
        $.ajax({
            type: "POST",
            async: false,
            url: "NewBuyerSupplierWizard.aspx/SaveServerDetails",
            data: data2send,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                try {  var res = Str(response.d); GetServerDetails(); Metronic.unblockUI();  }
                catch (err) { toastr.error("Lighthouse eSolutions Pte. Ltd.", 'Unable to add new server :' + err); Metronic.unblockUI(); }
            },
            failure: function (response) {  toastr.error("failure get", response.d); Metronic.unblockUI(); },
            error: function (response) { toastr.error("error get", response.responseText); Metronic.unblockUI();  }
        });
    ...
    var _servname = $('#txtServerName').val().trim(); var _servicepath = 'http://' +_domainval + "/LeSbuyerService_" +_servname; $('#txtServiceurl').val(_servicepath.trim());};
    /*end*/
    return { init: function () { handleNew_BuyerSupplier_Wizard();} };
