...
namespace DotNetBrowserWrapper
{
    public partial class NetBrowser : IDisposable
    {
        ...
        public Dictionary<string, object> dictCustomDialogueParams_OnAlert = new Dictionary<string, object>();
        ...
        public List<DOMElement> GetElementsbyId(string _tag,string _id)
        {
            List<DOMElement> _result = new List<DOMElement>();
           var _tags  = CurrentDocument.GetElementsByTagName(_tag);
           foreach (DOMElement a in _tags)
           {
               if (a.HasAttribute("id"))
               {
                   if (a.Attributes["id"] == _id)
                   {
                       _result.Add(a);
                   }
               }
        ...
        public DOMElement GetElementByType(string _type, string _value, string AttributeValue)//added by namrata 27-02-17
        {
            ...
            try
            {
                ...
                foreach (DOMElement i in inputs)
                {
                    ...
                    {
                        if (i.GetAttribute(AttributeValue) == _value)
                        {
                            _result = i;
                            break;
                        }
        ...
        public DOMElement GetElementByType(string _type, string _value, string AttributeValue,string idnValue,string idnAttValue)//added by namrata 27-02-17
        {
            DOMElement _result = null;
            try
            {
               ...
               foreach (DOMElement i in inputs)
                {
                    if (i.HasAttribute(AttributeValue))
                    {
                        ...
                        {
                            if (i.Attributes[idnAttValue] == idnValue)
                            {
                                _result = i;
                                break;
                            }
        ...
        public DOMElement GetElement(string className, string attributeName, string _value)
        {
            DOMElement ele = null;
            List<DOMNode> _eles = CurrentDocument.GetElementsByClassName(className);
            if (_eles != null)
            {
                foreach (DOMNode _d in _eles)
                {
                    List<DOMNode> _child = _d.Children;
                    int count = _child.Count;
                    if (count >= 4)
                    {
                        try
                        {
                            if (((DOMElement)_d.Children[3]).HasAttribute(attributeName))
                            {
                               if (((DOMElement)_d.Children[3]).Attributes[attributeName].ToString() == _value)
                                {
                                    ele = (DOMElement)_d.Children[3];
                                    break;
                                }
                            }
                        }
        ...
        public DOMElement GetElementByTag(DOMElement _ele, string Tag, string attributeName, string _value, string childTag)
        {
            DOMElement ele = null; List<DOMNode> _eles = null;
            if (_ele == null)
                _eles = CurrentDocument.GetElementsByTagName(Tag);
            else _eles = _ele.GetElementsByTagName(Tag);

            if (_eles != null)
            {
                foreach (DOMNode _d in _eles)
                {
                    string outerhtml = ((DOMElement)((DOMElement)_d).Parent).InnerHTML;
                    try
                    {
                        if (((DOMElement)_d).HasAttribute(attributeName))
                        {
                             //if (((DOMElement)_d).Attributes[attributeName].ToString() == _value)
                            if (outerhtml.Contains(_value))
                            {
                                List<DOMNode> _child = _d.Children;
                                foreach (DOMNode _c in _child)
                                {
                                    if (_c.TextContent != "\n")
                                    {
                                        ele = ((DOMElement)_c).GetElementByTagName(childTag);
                                        if (ele != null) break;
                                    }
                                }
                            }
       ...
       public List<DOMElement> GetElementsByTagName(string Tag, string attrName, string attrValue)
        {
            List<DOMElement> lstEle = new List<DOMElement>();
            try
            {
                var inputs = CurrentDocument.GetElementsByTagName(Tag);
                if (inputs.Count > 0)
                {
                    foreach (DOMElement i in inputs)
                    {
                        ...
                        {
                            if (i.GetAttribute(attrName) == attrValue)
                            {
                                lstEle.Add(i);
                            }
                        }
       ...
       public DOMElement GetElementByTagName(string Tag, string attrName, string attrValue, string Text, string childTag, string childAttrName, string childAttrVale, string childTagToFind)
       {
           DOMElement _ele = null;
           List<DOMElement> lstEle = GetElementsByTagName(Tag, attrName, attrValue);
           if (lstEle.Count > 0)
           {
               foreach (DOMElement ele in lstEle)
               {
                   string outerhtml = ((DOMElement)ele).InnerHTML;
                   if (outerhtml != "")
                   {
                       if (outerhtml.Contains(Text))
                       {
                           _ele = GetElementByTag(ele, childTag, childAttrName, childAttrVale, childTagToFind);
                           if (_ele != null) break;
        ...
        public bool ClickElement(DOMElement _element)
        {
            bool _result = true;
        ...
        public bool ClickElementbyType(DOMElement _element, string ele, string value, string attributeValue)
        {
            bool _result = false;
            if (_element == null) { return false; }
            _element.Click();
            Thread.Sleep(1000);
            DOMElement _eleRes = GetElementByType(ele, value, attributeValue);
            if (_eleRes != null)
            {
                _result = true;
            }
        ...
        public bool ClickElement(DOMElement _element, bool ReLoad, string _validate = "")
        {
            ...
            if (ReLoad)
            {
                ...
                {
                    LogText = e.ValidatedURL;
                    _browser_FinishLoadingFrame(ref currentdocloaded, _validate, false, false, _waitEvent, e);
                };
                _browser.FinishLoadingFrameEvent += eventHandler;
                _element.Click();
        ...
        public bool ClickElement(DOMElement _element, bool ReLoad,string validateText, string tag ,string attName,string attValue)
        {
            ...
            if (ReLoad)
            {
                ...
                {
                    LogText = e.ValidatedURL;
                    _browser_FinishLoadingFrame(ref currentdocloaded, validateText, false, false, _waitEvent, e);
                };
                _browser.FinishLoadingFrameEvent += eventHandler;
                _element.Click();
                ...
                if (!currentdocloaded)
                {
                    CurrentDocument = _browser.GetDocument();
                    currentdocloaded = true;
                    List<DOMNode> _ele = _browser.GetDocument().GetElementsByTagName(tag);
                    if (_ele != null)
                    {
                        foreach (DOMNode _d in _ele)
                        {
                            string a = ((DOMElement)_d).TextContent.Trim();
                           if (((DOMElement)_d).TextContent.Trim().Contains(attValue))
                                {
                                    currentdocloaded = true;
                                    break;
                                }
        ...
        public bool ClickElement(bool reLoadDoc, bool hasDialog, DOMElement _element)//23-11-2017
        {
            bool _result = true;
            CustomDialogHandler _CustomDialogHandler = null;
            dictCustomDialogueParams_OnAlert.Clear();
            bool docloaded = false;
            if (_element == null) { return false; }
            if (hasDialog)
            {
            ...
            if (hasDialog)
            {
                while (!docloaded) { Thread.Sleep(1000); }
            }

            if (hasDialog)
            {
                if (_CustomDialogHandler.customDialogueParams_OnAlert != null) dictCustomDialogueParams_OnAlert = _CustomDialogHandler.customDialogueParams_OnAlert;
            }
            _browser.DialogHandler = null;
            return _result;
        }
       ...
       public string ExecuteJavaScript(string FunctionName, object[] Value)
        ...
        private void SetScreenSize()
        {
           ...
           // browserView.Browser.SetSize(viewWidth, viewHeight);
            browserView.Browser.SetSize(viewWidth, 1024);
        }
        ...
        public bool SelectElementItem(DOMElement selectElement, int selectIndex)
        {
            bool success = false;
            try
            {
        ...
        public bool WaitForLoading(string nClass, string tag, string attribute, string _val)
        {
            int c = 0, i = 60000;
            bool _result = false; //simmy
    ...
    class CustomDialogHandler : DialogHandler
    {
        ...
        public Dictionary<string, object> customDialogueParams_OnAlert = new Dictionary<string, object>(); //Added Sayli
