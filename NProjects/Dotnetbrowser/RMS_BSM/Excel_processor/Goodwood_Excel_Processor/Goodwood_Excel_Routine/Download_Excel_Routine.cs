...
namespace Excel_SPL_Routine
{
    public class Download_Excel_Routine
    {
       ...
       Dictionary<int ,List<string>> _ItemDetails = new Dictionary<int,List<string>>();
                ...
                if (_itemStart > 0)
                {
                    ...
                    for (int i = _itemStart; i < _ExcelRTN._worksheet.Cells.MaxDataRow; i++)
                    {
                        ...
                        if (convert.ToInt(ItemNo.Trim()) > 0)
                        {
                            ...
                            //_ItemRmks = Get_Item_Value_Qualifier("REMARKS", _ItemMappings, i);//10-04-18
                            _ItemRmks = Get_Item_Value_Qualifier("ITEM_REMARKS", _ItemMappings, i);//10-04-18
        ...
        #region commented by kalpita on 09/08/2019
        //public void Get_Item_details(ITEM_MAPPINGS _Mapping)
        //{
        //    List<string> lststr = null;
        //    try
        ...
        //                    if (!string.IsNullOrEmpty(_PartDesc))
        //                    {
        //                        if (nItmCtr > convert.ToInt(ItemNo.Trim())) { ItemNo = (nItmCtr + 1).ToString(); }
        ...
        public bool GenerateRFQMTML()
        {
            ...
            try
            {
                ...
                else throw new Exception("Element RFQ No. not found.");
                {
                    ...
                    //  Process_File_Name = Path.GetFileNameWithoutExtension(Process_File_Name) + "_" + PortCode + Path.GetExtension(Process_File_Name);//added by kalpita on 05/03/2020 (append port to file)
                    string cXlsFile_Name = Path.GetFileNameWithoutExtension(Process_File_Name) + "_" + PortCode + Path.GetExtension(Process_File_Name);//changed by kalpita on 11/03/2020
                    _docHeader.MessageReferenceNumber ="RequestForQuote : " + BuyerRef;//28-08-2017
                    _docHeader.MessageNumber = Process_File_Name;// BuyerRef;//28-08-2017
                    _docHeader.OriginalFile = cXlsFile_Name;
