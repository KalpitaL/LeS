...
namespace DotNetBrowserWrapper
{
    public partial class NetBrowser : IDisposable
    {
        ...
        private void WriteLog(string _logText, string _logFile = "")
        {
            ...
            if (_logpath == null || _logpath == "") _logpath = "Log";
       ...
       public string ExecuteJavaScript(string FunctionName, object[] Value)
        ...
        //public bool WaitForLoading(string nClass, string attribute, string attValue)
        //{
        //    bool _result = false; //simmy
        //    int _wait = 5; int i = 0;
        //    if (_waitPeriod > 0) { _wait = _waitPeriod; }
        ...
        public void Dispose()
        {
            try
            {
                this.LogText = "Disposing (browserView)";
                browserView.Dispose();

                this.LogText = "Disposing (browserView.Browser)";
                browserView.Browser.Dispose();

                this.LogText = "Disposing (browser)";
                _browser.Dispose();

                if (printbrowser != null && !printbrowser.IsDisposed())
                {
                    this.LogText = "Disposing (printbrowser)";
        ...
        public NetBrowser()
        {
            ...
            {
                this.LogText = "Error at NetBrowser constructor: " + ex.Message.ToString();
            }
        }
