    ...
    public class PacificRoutine
    {
        ...
        string strLogPath = "", strAuditPath = "", strProcessorName = "", strScreenShotPath = "", strSupplier = "", strBuyer = "", strAttachmentPath = "",
           strRFQPath = "", strExcelPath = "", strMailTextPath = "", strMTMLUploadPath = "", strDocType = "", strMessageNumber = "",
            ...
            strDepositCost = "", strOtherCost = "", strURL = "", strToken="",docType="", cAckUpd = "",strAuditName="";
      ...
      //  Dictionary<string, string> dicURLToken = null;
        ...
        public void Read_MailTextFiles()
        {
            ...
            {
                string sFile = strScreenShotPath + "\\" + strAuditName + "_" + this.docType + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + strSupplier + ".pdf";
        ...
        {
            RFQ_mailFiles.Clear(); PO_mailFiles.Clear();
                        ...
                        if (_eleErrMsg != null)
                        {
                            ...
                            if (_h2.Count == 1)
                            {
                                if (_h2[0].TextContent.Contains("request could not be found or is no longer valid"))
                                {
                                    ...
                                    string sFile = strScreenShotPath + "\\" + strAuditName + "_Login" + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + strSupplier + ".pdf";
                                    _netWrapper.PrintPDF(sFile, false);
                    ...
                    if (GetRFQItems(strExcelRFQ))
                    {
                        if (GetAddress())
                        {
                            ...
                            else
                            {
                                ...
                                CreateAuditFile(lesXMLFile, strProcessorName, arrRFQNo[0], "Downloaded", Audit);
                                string sFile = strScreenShotPath + "\\" + strAuditName + "_RFQ" + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + strSupplier + ".pdf";
                                _netWrapper.PrintPDF(sFile, false);
        ...
        public bool GetRFQHeader(string strRFQNo, string strRemarksTitle, string strURL, string strToken)
        {
            ...
            try
            {
                ...
                _lesXml.Dialect = strAuditName;//"Pacific_Basin"
                _lesXml.Version = "1";
        ...
        {
            Quote_mtmlFiles.Clear(); POC_mtmlFiles.Clear();
            ...
            foreach (FileInfo _MtmlFile in _Files)
            {
                ...
                else
                {
                    ...
                    _netWrapper.LogText = "Invalid doctype : " + strDocType + " for ref " + strUCRefNo;
                    MoveFileToError(_MtmlFile.FullName, "LeS-1004.1:Unable to process file due to invalid doc type " + strDocType + " for " + strUCRefNo, "Error");
        ...
        public void ProcessQuoteMTML(string MTML_QuoteFile)
        {
                                ...
                                if (FillAdditionalChanrges())
                                {
                                            ...
                                            else if (strBuyerTotal != "")//16-12-2017
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (_diff <= 1)
                                                        ...
                                                        _netWrapper.LogText = "Unable to upload Quote for Ref : '" + strUCRefNo + "' due to Total Amount '" + strGrandTotal + "' mismatch on site Grand Total '" + txtTotal.InnerText + "'.";
                                                    MoveFileToError(MTML_QuoteFile, "LeS-1008.1:Unable to Save Quote due to amount mismatch for " + strUCRefNo, "Quote");
                                                }
                                            }
                                            else
                                            {
                                    ...
                                    else
                                    {
                                       // MoveFileToError(MTML_QuoteFile, "Unable to fill header discount for Ref  " + strUCRefNo, "Quote");
                                        _netWrapper.LogText = "Unable to fill header discount for Ref  " + strUCRefNo;
                                        MoveFileToError(MTML_QuoteFile, "LeS-1007:Unable to Save file for Quote - header discount Field(s) not present for " + strUCRefNo, "Quote");
                                    }
                                }
                                else
                                {
                                    //MoveFileToError(MTML_QuoteFile, "Unable to fill additional chargesheader discount for Ref  " + strUCRefNo, "Quote");
                                    _netWrapper.LogText = "Unable to fill additional chargesheader discount for Ref  " + strUCRefNo;
                                    MoveFileToError(MTML_QuoteFile, "LeS-1007:Unable to Save file for Quote - additional chargesheader discount Field(s) not present for " + strUCRefNo, "Quote");
                                }
                            }
                            else
                            {
                                //MoveFileToError(MTML_QuoteFile, "Unable to fill header delivery days for Ref  " + strUCRefNo, "Quote");
                                _netWrapper.LogText = "Unable to fill header delivery days for Ref  " + strUCRefNo;
                                MoveFileToError(MTML_QuoteFile, "LeS-1007:Unable to Save file for Quote - header delivery days Field(s) not present for " + strUCRefNo, "Quote");
                            }
                        }
                    }
                    _netWrapper.browser.LoadURL("about:blank"); Thread.Sleep(1500);
            ...
            catch (Exception ex)
            {
                ...
                _netWrapper.LogText = "Exception while processing Quote MTML : " + ex.GetBaseException().ToString();
                                ...
                                try
                                {
                                    ...
                                    if (_data.Count > 0)
                                    {
                                        ...
                                        if (_item != null)
                                        {
                                            ...
                                            {
                                                if (string.IsNullOrEmpty(_item.MeasureUnitQualifier) || (_item.MeasureUnitQualifier == "PC") || (_item.MeasureUnitQualifier == "PCS")) { _item.MeasureUnitQualifier = "PCE"; }//added by Kalpita on 04/10/2019
            ...
            try
            {
                ...
                if (_eleSaveQuote != null)
                {
                    ...
                    if (_netWrapper.ClickElementbyType(_eleSaveQuote, "label", pagevar.SaveStatusId, "id"))
                    {
                        ...
                        {
                            string sFile = strScreenShotPath + "\\" + strAuditName + "_Quote_Draft_" + strUCRefNo.Replace('/', '_') + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + strSupplier + ".pdf";
                            _netWrapper.PrintPDF(sFile, false);
                                    ...
                                    if (_netWrapper.ClickElement(_eleSendQuote))
                                    {
                                        ...
                                        if (msg != "" && msg.Contains("Quotation successfully sent"))
                                        {
                                            if (strURL != "" && strToken != "")
                                            {
                                                ...
                                                {
                                                    sFile = strScreenShotPath + "\\"+ strAuditName +"_Quote_Submit_" + strUCRefNo.Replace('/', '_') + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + strSupplier + ".pdf";
                                                    _netWrapper.PrintPDF(sFile, false);
                                                }
                                            }
        ...
        {
            string sFile = strScreenShotPath + "\\" + strAuditName + "_" + DocType + "_Error_" + strUCRefNo.Replace('/', '_') + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + strSupplier + ".pdf";
            _netWrapper.PrintPDF(sFile, false);
            ...
            if (File.Exists(strMTMLUploadPath + "\\Error\\" + Path.GetFileName(MTML_QuoteFile)))
                CreateQuoteAuditFile(sFile, strAuditName + " " + DocType, strUCRefNo, "Error", message);
        }
        ...
        public void MoveFileToBackup(string MTML_QuoteFile, string message)
        {
            ...
            if (File.Exists(strMTMLUploadPath + "\\Backup\\" + Path.GetFileName(MTML_QuoteFile)))
                CreateQuoteAuditFile(MTML_QuoteFile, strAuditName + " Quote", strUCRefNo, "Success", message);
            _netWrapper.LogText = message;
        }
        ...
        public void ProcessPO()
        {
            try
            {
                ...
                {
                    _netWrapper.LogText = "No PO files found to process.";
                }
                    ...
                    if (GetPOItems())
                      {
                          if (GetPOAddress())
                          {
                              ...
                              else
                              {
                                  ...
                                  CreateAuditFile(lesXMLFile, strProcessorName, arrPONo[0], "Downloaded", Audit);
                                  string sFile = strScreenShotPath + "\\" + strAuditName + "_PO" + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + strSupplier + ".pdf";
                                  _netWrapper.PrintPDF(sFile, false);
                                  if (File.Exists(strMailTextPath + "\\Error\\" + Path.GetFileName(MailTextFile))) File.Delete(strMailTextPath + "\\Error\\" + Path.GetFileName(MailTextFile));
                                  File.Move(MailTextFile, strMailTextPath + "\\Error\\" + Path.GetFileName(MailTextFile));
                              }
        ...
        public void ProcessPOCMTML(string MTML_QuoteFile)
        {
                ...
                if (NavigateToURL(strURL, strToken, Path.GetFileName(MTML_QuoteFile), strMTMLUploadPath) && strUCRefNo != "")
                {
                    if (!IsDisabledPOCElement())
                    {
                            ...
                            if (Fill_POCHeader_Details())
                            {
                                if (Fill_POCItem_Details(MTML_QuoteFile))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        CreateQuoteAuditFile(MTML_QuoteFile,  strAuditName + "_POC", "", "Error", "Decline POC " + Path.GetFileName(MTML_QuoteFile) + " found for buyer " + strBuyerName + ", move to error.");
                        ...
                        else
                        {
                            ...
                            MoveFileToError(MTML_QuoteFile, "LeS-1008.5:Unable to Save Quote since quote total is zero for " + strUCRefNo, "POC");
                        }
                    }
                    else
                    {
            ...
            catch (Exception ex)
            {
                ...
                MoveFileToError(MTML_QuoteFile, "LeS-1004:Unable to process file due to " + ex.GetBaseException().ToString(), "POC");
            }
        }
        ...
        {//PacificB
            string sFile = strScreenShotPath + "\\" + strAuditName + "_POC_Draft_" + strUCRefNo.Replace('/', '_') + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + strSupplier + ".pdf";
            _netWrapper.PrintPDF(sFile, false);
            DOMElement _eleConfirm = _netWrapper.GetElementByType("button", pagevar.clkbtnConfirmOrder, "click.delegate");
            if (_eleConfirm != null)
            {
                if (_netWrapper.ClickElement(_eleConfirm))
                {
                    ...
                    if (msg != "" && msg.Contains("Purchase Order successfully confirmed"))
                    {
                        if (strURL != "" && strToken != "")
                        {
                            ...
                            if (NavigateToURL(strURL.Trim(), strToken.Trim(), "", ""))
                            {
                                sFile = strScreenShotPath + "\\" + strAuditName +"_POC_Submit_" + strUCRefNo.Replace('/', '_') + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + strSupplier + ".pdf";
                                _netWrapper.PrintPDF(sFile, false);
                            }
                        }
                        MoveFileToBackup(MTML_QuoteFile, msg);
                    ...
                    {
                        //MoveFileToError(MTML_QuoteFile, "Unable to confirm order due to " + msg + " for Ref No: " + strUCRefNo, "POC");
                        _netWrapper.LogText = "Unable to confirm order due to " + msg + " for Ref No: " + strUCRefNo;
                        MoveFileToError(MTML_QuoteFile, "LeS-1004:Unable to process file due to " + msg + " for " + strUCRefNo, "POC");
                    }
                }
            }
        }
