    ...
    public class Http_Download_Routine : LeSCommon.LeSCommon
    {
        string BuyerLinkCode = "", SuppLinkCode = "", AttachmentPath = "", CurrentURL = "", AuditDocPath = "", DATE_FORMAT = "dd-MMM-yyyy", isPortCheck = "false",
           ...
           supplierPhone = "", supplierEmail = "", supplierFax = "", VesselName = "", PortName = "", PortCode = "", SupplierComment = "", PayTerms = "",
            ...
            BCode = "", SCode = "", DoneStatusPath = "",_errorLog="",cSecureSite,cModuleName="";
        string[] QuotePath;
        int RFQ_Counter = -1, IsAltItemAllowed = 0, IsPriceAveraged = 0, IsUOMChanged = 0,nQuoteCounter=0, nFileRetry=0;
        string[] settings = null;
        double extraItemsAmt = 0, additionalCost_freight = 0;

        bool SaveDoc = true, isQuote = false, IsDecline = false, isSaveAsDraft = false, isSubmitQuote = false, isDoneStatus = false,isChangedUrl=false,IsMovedToError=false;
        ...
        public void SetConfigSettings()
        {
            ...
            nFileRetry = (ConfigurationManager.AppSettings["FILE_RETRY"] != "") ? Convert.ToInt32(ConfigurationManager.AppSettings["FILE_RETRY"]) : 0;//added by kalpita on 24/10/2019
        }
        ...
        public void ReadRFQFromMail()
        {
            ...
            try
            {
                cModuleName = "MTM_HTTP_RFQ_Downloader";
                SetConfigSettings();
                orgAttachFile = null;
                lstUrls.Clear();
                ...
                if (RFQ_Counter > -1)
                {
                    ...
                    if (CurrentURL.Trim().Length > 0)
                    {
                        try
                        {
                            ...
                            {
                                CurrentURL = GetOriginal_ProtectedURL(URL);
                                ProcessRFQ();
                            }
                            else
                            {
            ...
            try
            {
                ...
                if (MTM_DOMAIN.Trim().Length > 0)
                {
                    ...
                    if (txt.Lines.Length > 0)
                    {
                            ...
                            if (Idx > 0)
                            {
                                ...
                                if (endIdx > 0)
                                {
                                    ...
                                    if (LoadURL("input", "id", "btnSubmit"))
                                    {
                                        ...
                                        string _startUrl = this.URL.StartsWith("https") ? "https" : "http";//added by Kalpita on 05/02/2020
                                        this.URL = _startUrl + "://portal.mtmsm.com/public/pos/" + this.URL.Replace("./", "").Trim('"').Trim().Replace("&amp;", "&").Replace(Environment.NewLine, "");
                                        isChangedUrl = true;
                                    }
                                    else this.URL = null;
                                }
                        ...
                        else if (txt.Text.Contains("https://apc01.safelinks.protection.outlook.com"))//added on 22042020
                        {
                            int Idx = txt.Text.IndexOf("\"" + "https://apc01.safelinks.protection.outlook.com");
                            if (Idx == -1) Idx = txt.Text.IndexOf("<" + "https://apc01.safelinks.protection.outlook.com");
                            if (Idx > 0)
                            {
                                int endIdx = txt.Text.Substring(Idx + 1).Trim().IndexOf("\"");
                                int greatInx = txt.Text.Substring(Idx + 1).Trim().IndexOf(">");
                        ...
                        if (this.URL == null || this.URL == "")
                        {
                            ...
                            else if (urlStartIdx != 0)
                            {
                                ...
                                if (urlIdx == -1)
                                {
                                    urlIdx = txt.Text.IndexOf("<" + MTM_DOMAIN);                                    
                                }
                                ...
                                {
                                    //added by kalpita on 28/11/2019
                                    string Lnktxt = GetURL(txt, MTM_DOMAIN); if (string.IsNullOrEmpty(Lnktxt)) { urlIdx = -1; }
                                    else
                                    {
                                        this.URL = Lnktxt.Split('|')[0]; urlIdx = Convert.ToInt16(Lnktxt.Split('|')[1]);
        ...
        //added by Kalpita on 28/02/2020
        private string GetOriginal_ProtectedURL(string CurrURL)
        {
            if (CurrURL.Contains("protect-de.mimecast.com"))
            {
                string _cActnurl = _httpWrapper.GetElement("form", "id", "form1").GetAttributeValue("action", "").Trim();
                string _startUrl = _cActnurl.StartsWith("https") ? "https" : "http";
                return _startUrl + "://portal.mtmsm.com/public/pos/" + _cActnurl.Replace("./", "").Trim('"').Trim().Replace("&amp;", "&").Replace(Environment.NewLine, "");
            }
            else { return CurrURL; }
        }

        //added by kalpita on 02/12/2019
        private string GetURL(RichTextBox txtData,string domain)
        {
            string _res = ""; int urlindx=-1;
            try
            {
                for (int k = 0; k < txtData.Lines.Length; k++)
                {
                    string line = Convert.ToString(txtData.Lines[k]);
                    if (line.Contains(domain)) //&& line.Contains("http")
                    {
                        if (line.StartsWith("http")) { _res = line.Trim().Replace("&amp;", "&"); urlindx = k; }
                        else if (line.Contains("http"))
                        {
                            urlindx = k;
                            //string chk = GetURL_FromHtml(MTM_DOMAIN, Lnktxt, urlIdx);
                            string urlpattern = @"https?:\/\/[^ ]*>";
                            string[] _arr = Regex.Matches(line, urlpattern, RegexOptions.Multiline)
                                            .Cast<Match>()
                                            .Select(m => m.Value).ToArray();
                            _res = _arr[0].Replace("\"", "").Replace(">", "").Trim().Replace("&amp;", "&");

                        }///added by kalpita on 06/02/2020
                        break;
                    }
                }
            }
        ...
        private void GetBuyerSupplierDetail(string SuppDomain)
        {
            try
            {
                SupplierCode = ""; BuyerLinkCode = ""; SuppLinkCode = ""; isPortCheck = "";//added by Kalpita on 27/02/2020
        ...
        public void GetCommonDetails(ref MTMRFQ _mtmRFQ, ref string VRNO)
        {
            ...
            _mtmRFQ.RFQ.MessageNumber = CurrentURL.Replace("http","https");//changed by kalpita on 07/02/2020
                ...
                if (_ncTable != null)
                {
                    ...
                    if (_nodes.Count > 0)
                    {
                        ...
                        foreach (HtmlNode _node in _nodes)
                        {
                            ...
                            if (_tdNode != null)
                            {
                                if (_tdNode.ChildNodes.Count > 5)//added by kalpita on 04/01/2020
                                {
        ...
        private void DownloadFile(string VRNO, string LinkFile)
        {
            ...
            CreateAuditFile(LinkFile, "MTM_HTTP_RFQ_Downloader", VRNO, "Downloaded", "RFQ '" + VRNO + "' downloaded successfully.", BuyerCode, SupplierCode, AuditPath);//MTM_SHIP_RFQ
        ...
        private void MoveToNextRFQ(string errLog, string VRNO)
        {
            ...
            try
            {
                ...
                if (errLog.Contains("Invalid Link")) { cMsgNo = "LeS-1004.2:"; } else if (errLog.Contains("Link not")) { cMsgNo = "LeS-1004.5:"; } else { cMsgNo = "LeS-1004:"; }
        ...
        private void MoveToError(string Msg, string VRNO, string FileToMove, string screenShot)
        {
            if (Msg.Trim() != "") //changed by simmy 14022018 avoid audit with blank message.
            {
                ...
                if (screenShot.Trim().Length > 0)
                {
                    ...
                    CreateAuditFile(Path.GetFileName(screenShot),cModuleName , VRNO, "Error", Msg, BuyerCode, SupplierCode, AuditPath);//"MTM_HTTP_Processor"
                    else
                        CreateAuditFile(Path.GetFileName(screenShot).Replace(".png", ".html"), cModuleName, VRNO, "Error", Msg, BuyerCode, SupplierCode, AuditPath);
                }
                else
                    CreateAuditFile(Path.GetFileName(FileToMove), cModuleName, VRNO, "Error", Msg, BuyerCode, SupplierCode, AuditPath);
            }
        ...
        public void UploadQuote()
        {
            ...
            cModuleName = "MTM_HTTP_Quote_Processor";
            foreach (string quotePath in QuotePath)
            {
                if (quotePath.Trim().Length > 0)
                {
                    string[] quotes = Directory.GetFiles(quotePath.Trim(), "*.xml");
                    foreach (string quotefile in quotes)
                    {
                        LogText = "Processing of file - " + quotefile + " Started.";
                        ProcessMTMLFiles(quotefile);
                        #region commented by kalpita on 29/01/2020
                        //try
                        //{
                        ...
                        //{
                        //    nQuoteCounter++;
                        //    if (nQuoteCounter < nFileRetry)
                        //    {
                        //        ProcessMTMLFiles(quotefile, nQuoteCounter);
                        //    }
                        //    else
                        //    {
                        ...
                        //        PrintScreen(AttachmentPath + "\\" + eFile); MoveMTMLToError(quotefile);
                        //        CreateAuditFile(eFile, "MTM_HTTP_Quote_Processor", "", "Error", "LeS-1008:Unable to save quote due to " + ex.GetBaseException().Message.ToString(), BuyerCode, SupplierCode, AuditPath);
                        //    }
                        //}
        ...
        private Boolean ProcessMTMLFiles(string quotefile)
        {
            bool IsResult = false; string strAuditmsg = "";
            try
            {
                ...
                if (_interchange != null)
                {
                    ...
                    string cURL = MessageNumber.Trim().Replace("&amp;", "&");//changed by Kalpita on 03/07/2019
                    URL = (!cURL.StartsWith("https")) ? cURL.Replace("http", "https") : cURL;//added by kalpita on 11/02/2020
                    ...
                    {
                        this.URL = CheckLoadUrl(URL);         //changed by kalpita on 13/02/2020( url coming as http/https)              
                        if (isQuote)//check for declined6 quote
                        {
                            if (Convert.ToDouble(GrandTotal) > 0)
                            {
                                if (LoadURL("input", "id", "btnSubmit"))
                                {
                                    bool IsRequest = CheckRequestedBid(UCRefNo, quotefile);
                                    if (!IsRequest)
                                    {
                                            ...
                                            if (siteRefNo.InnerText.Trim().Replace(" ", "") == UCRefNo)
                                            {
                                                ...
                                                if (isSaveAsDraft)
                                                {
                                                    if (SaveQuotation(ref eFile, quotefile))
                                                    {
                                                        ...
                                                        CreateAuditFile(Path.GetFileName(eFile), "MTM_HTTP_Quote_Processor", UCRefNo, "Submitted", "Quote for RefNo '" + UCRefNo + "' saved successfully.", BuyerCode, SupplierCode, AuditPath);// "MTM_HTTP_Quote"
                                                ...
                                                else if (isSubmitQuote)
                                                {
                                                    if (SaveQuotation(ref eFile, quotefile))
                                                    {
                                                        ...
                                                        {
                                                            LogText = "Quote for RefNo '" + UCRefNo + "' saved successfully.";
                                                            Check_SubmitQuote(strAuditmsg, eFile, quotefile);//changed by Kalpita on 29/11/2019
                                                        }
                                                        else
                                                        {
                                                            LogText = "Quote for RefNo '" + UCRefNo + "' declined successfully.";
                                                            CreateAuditFile(Path.GetFileName(quotefile), "MTM_HTTP_Quote_Processor", UCRefNo, "Declined", "Quote for RefNo '" + UCRefNo + "' declined successfully.",
                        ...
                        else
                        {
                            ...
                            LogText = "Parameters not configured, Upload Quote flag is set as false for Buyer-Supplier (" + BuyerCode + "-" + SupplierCode + ").";
                        }
                    }
                }
            }
        ...
        private string CheckLoadUrl(string checkURL)
        {
            string cUrl = "";
            try
            {
            ...
            {
                this.URL= checkURL.Replace("https","http");
                if (LoadURL("input", "id", "btnSubmit"))
                {
                    cUrl = this.URL;
                }
        ...
        private void Check_SubmitQuote(string strAuditmsg, string eFile, string quotefile)
        {
            int ctr = 0;
            bool IsQteSubmitted = SubmitQuote(quotefile, ref eFile);
            if (IsQteSubmitted)
            {
                LogText = strAuditmsg = "Quote for RefNo '" + UCRefNo + "' submitted successfully.";
                CreateAuditFile(Path.GetFileName(eFile), "MTM_HTTP_Quote_Processor", UCRefNo, "Submitted", strAuditmsg, BuyerCode, SupplierCode, AuditPath);
                MoveToBackup(quotefile);
                if (isDoneStatus)
            ...
            else
            {
                ...
                {
                    ctr++;
                    LogText = "Quote Submit retry";
                    IsQteSubmitted = SubmitQuote(quotefile, ref eFile);
                }
            }
        }
      
        private Boolean  CheckRequestedBid(string UCRefNo,string quotefile)
        {
            string eFile = ""; bool IsRequestedBid = false;
            HtmlNode _dvModal = _httpWrapper.GetElement("div", "id", "dvmodal");
            if (_dvModal != null && _dvModal.InnerText.Trim().Contains("OOPS"))//check if bid is close or not
            {
                IsRequestedBid = true;
        ...
        private bool SaveQuotation(ref string eFile, string quoteFile)
        {
            ...
            {
                LogText = "Error at SaveQuotation() : " + ex.GetBaseException().Message.ToString();
                result = false;
            }
            return result;
        }
                ...
                if (Currency != "")
                {
                    ...
                    if (PostURL("select", "name", "ddlCurrency"))
                    {
                        ...
                        if (_currency != null)
                        {
                            ...
                            {
                                _currency.SetAttributeValue("value", Currency);
                                currSuccess = true;
                                LogText = "Currency saved.";
                            }
                        }
        ...
        public bool SetLineItemDetails(string strURL)
        {
            ...
            try
            {
               ...
               // URL = "http://portal.mtmsm.com/public/pos/QuoteManager.aspx/Update_unitpriceAndquantity";
                URL = strURL.Split('?')[0] + "/Update_unitpriceAndquantity";//added by kalpita on 05/02/2020
                URL = URL.Replace("quotemanager", "QuoteManager");
...
//private Boolean ProcessMTMLFiles_Old(string quotefile, int nQuoteCounter)
//{
//    bool IsResult = false; string strAuditmsg = "";
...
//changed by Kalpita on 29/11/2019
...
//        else
//        {
//            if (isQuote)//check for declined quote
//            {
//                if (Convert.ToDouble(GrandTotal) > 0)
//                {
//                    if (LoadURL("input", "id", "btnSubmit"))
