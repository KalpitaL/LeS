...
namespace Http_Asiatic_Routine
{
    public class Http_Download_Routine : LeSCommon.LeSCommon
    {
        string sAuditMesage = "", MailTextFilePath = "", DocType = "", VRNO = "", PO_Path = "", QuotePath = "", errStr = "", AuditName = "", BuyerName = "", SupplierName = "",
            FileType = "", EncyptUrls = "", cMsgFile="",NoAuditName="";
        ...
        public void Read_MailTextFiles()
        {
            ...
            try
            {
                ...
                AuditName = NoAuditName;//added by Kalpita on 27/11/2019
                if (_dir.GetFiles().Length > 0)
                {
                    FileInfo[] _Files = _dir.GetFiles();
                    foreach (FileInfo _MtmlFile in _Files)
                    {
                        filename = _MtmlFile.FullName; cMsgFile = Path.GetFileNameWithoutExtension(filename) + ".msg";
                        URL = GetURL(filename);
        ...
        //{
        //    string _url = "";
        //    for (int i = startIndex; i < _txtData.Lines.Length; i++)//
        //    {
        //        string line = _txtData.Lines[i];
        //        int startindx = line.IndexOf("<"); _url = line.Substring(startindx);
            ...
            try
            {
                if (URL != "")
                {
                    ...
                    if (!slProcessedItem.Contains(URL))
                    {
                        ...
                        {
                          //  string eFile = PrintScreenPath + AuditName + "\\+_" + this.DocType + "Error_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + SupplierCode + ".png";
                            string eFile = PrintScreenPath + "\\" + AuditName + "_" + this.DocType + "Error_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + SupplierCode + ".png";//changed by kalpita on 26/06/2019
        ...
        //changed by kalpita on 28/06/2019(Asiatic),27/11/2019
        private string CheckAuditName(RichTextBox txtData)
        {
            string cAName = "";
            string AuditNameDetails = ConfigurationManager.AppSettings["AUDITNAME"].Trim();
            if (AuditNameDetails != null)
            {
                string[] _adtdet = AuditNameDetails.Split('|');
                foreach (string _rdomain in _adtdet)
                {
                    for (int i = 0; i < _txtData.Lines.Length; i++)
                    {
                        if (_txtData.Lines[i].ToUpper().Contains(_rdomain.ToUpper()))
                        {
                            cAName = _rdomain;
                            break;
                        }
            ...
            if (string.IsNullOrEmpty(cAName)) { cAName = NoAuditName; }
            return cAName;
        }
        ...
        public void WriteErrorLog_With_Screenshot(string AuditMsg, string Filename)
        {
           // if (string.IsNullOrEmpty(AuditName)) { AuditName = NoAuditName; }//changed by kalpita on 28/06/2019(Asiatic),27/11/2019            
            ...
            {
                string eFile = PrintScreenPath + "\\" + AuditName + "\\_" + this.DocType + "Error_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + SupplierCode + ".png";//changed by kalpita on 01/11/2019
                if (!PrintScreen(eFile)) eFile = "";
                {
                    if (this.DocType != "PO")
                        CreateAuditFile(eFile, AuditName+"_HTTP_" + this.DocType, VRNO, "Error", AuditMsg, BuyerCode, SupplierCode, AuditPath);
                    else
                        CreateAuditFile(eFile, AuditName+"_HTTP_" + this.DocType, VRNO, "Error", AuditMsg, BuyerCode, SupplierCode, AuditPath);
                }
            }
        ...
        public void WriteErrorLog_With_Screenshot(string AuditMsg, string Filename, string ErrorNo)
        {
           // if (string.IsNullOrEmpty(AuditName)) { AuditName = NoAuditName; }//changed by kalpita on 28/06/2019(Asiatic),27/11/2019
            ...
            if (!AuditMsg.Contains("(404) Not Found"))
            {
                ...
                {
                    if (this.DocType != "PO")
                        CreateAuditFile(eFile, AuditName + "_HTTP_" + this.DocType, VRNO, "Error", ErrorNo + AuditMsg, BuyerCode, SupplierCode, AuditPath);
                    else
                        CreateAuditFile(eFile, AuditName + "_HTTP_" + this.DocType, VRNO, "Error", ErrorNo + AuditMsg, BuyerCode, SupplierCode, AuditPath);
                }
            }
