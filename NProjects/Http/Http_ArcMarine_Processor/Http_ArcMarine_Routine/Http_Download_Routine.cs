...
namespace Http_ArcMarine_Routine
{
    public class Http_Download_Routine : LeSCommon.LeSCommon
    {
        ...
        string MapPath = "", LinkPath = "", ImagePath = "", QuotePOCPath = "", errLog = "", VRNO = "", MsgFile = "", orgEmlFile = "", BuyerName = "", currentURL = ""
            , FileType = "", currentXMLFile = "", UCRefNo, cQuoteURL, cPrevPageData, cViewStateGen, cViewState, cEventArg, cScrollYPst, cOrgURL;
        ...
        public void ProcessQuoteMTML(string MTMLFile)
        {
            ...
            try
            {
                if (UCRefNo != "")
                {
                    ...
                    URL = MessageNumber;
                    string orgURL = convert.ToString(MessageNumber).Trim();
                    ...
                    if (LoadURL("span", "id", "Title1_lblTitle"))
                    {
                      ...
                      //  FillItems_check(orgURL, orgRFQURL);
        ...
        private void FillItemDetails(HtmlNodeCollection _tr, List<LineItem> _extraItems, Dictionary<int, LineItem> _items, 
            out double extraItemCost, HtmlAgilityPack.HtmlDocument doc)
        {
                                                ...
                                                if (iRemarks.Trim().Length > 0)
                                                {
                                                    ...
                                                    if (_inpRemarks != null)
                                                    {
                                                        ...
                                                        if (_startIndx > 0)
                                                        {
                                                            ...
                                                            if (_endIndx > 0) _itemRemURL = onClickEvent.Substring(_startIndx, _endIndx);
                                                        }

                                                        if (cOrgURL.Trim().Length > 0)
                                                        {
                                                            _itemRemURL = fixedUrl + "ArchangelReports/Purchase/" + _itemRemURL;
                                                            _itemRemURL = _itemRemURL.Replace("&amp;", "&");
                                                        }
                                                    }

                                                    _itemRemarks.Add(iObj.Number + "|" + _itemRemURL, convert.ToString(iObj.LineItemComment.Value).Trim());
                                                }
            ...
            foreach (KeyValuePair<string, string> pair in _itemRemarks)
            {
                ...
                this.URL = remURL;
                bool iRemPageLoaded = LoadURL("a", "id", "MenuLineItemDetail_dlstTabs_ctl00_btnMenu");
                if (iRemPageLoaded)
                {
                    ...
                    if (LoadURL("a", "id", "MenuLineItemDetail_dlstTabs_ctl00_btnMenu"))
                    {
                        ...
                        string postData = boundary + Environment.NewLine + "Content-Disposition: form-data; name=\"ToolkitScriptManager1_HiddenField\"" + Environment.NewLine + Environment.NewLine + Environment.NewLine +
                                                         ...
                                                         Environment.NewLine + boundary + Environment.NewLine + "Content-Disposition: form-data; name=\"_content_txtItemDetails_ucCustomEditor_ctl02\"" + Environment.NewLine + Environment.NewLine + HttpUtility.HtmlEncode(Remarks.Trim()) + Environment.NewLine +
                        ...
                        bool remUpdated = _httpWrapper.PostURL(URL, postData, "a", "id", "MenuLineItemDetail_dlstTabs_ctl00_btnMenu");


                        if (remUpdated) LogText = "Item Remarks Updated for -" + itemNo;
...
// public void FillItemRemarks_old(string iRemarks, HtmlNode _inpRemarks, string ItemNo)
// {
//     string cRmkURL = "";
//     if (iRemarks.Trim().Length > 0)
//     {
