...
{
    public class HTTP_KGJS_Routine : LeSCommon.LeSCommon
    {
        ...
        public void LoadAppsettings()
        {
            ...
            catch (Exception e)
            {
               // sAuditMesage = "Exception in LoadAppsettings: " + e.GetBaseException().ToString();
                LogText = sAuditMesage = "Parameters not configured";//changed by kalpita on 21/10/2019
               // CreateAuditFile("", "KGJS_HTTP", VRNO, "Error", "LeS-1012:"+sAuditMesage, BuyerCode, SupplierCode, AuditPath);
            }
        }
                ...
                if (_dir.GetFiles().Length > 0)
                {
                    ...
                    foreach (FileInfo _File in _Files)
                    {
                        ...
                        if (URL != "")
                        {
                            if (_domainIndex >= 0)
                            {
                                ...
                                catch (Exception e)
                                {
                                    ...
                                    WriteErrorLog_With_Screenshot("Unable to process file due to " + e.GetBaseException().Message.ToString(), filename, "LeS-1004:");
                            ...
                            else
                            {
                                ...
                                CreateAuditFile(Path.GetFileName(filename), "KGJS_RFQ", "", "Error", "LeS-1004:Unable to process file " + Path.GetFileName(filename), BuyerCode, SupplierCode, AuditPath);
                                if (File.Exists(HTMLMailTextPath + "\\Error\\" + Path.GetFileName(filename))) File.Delete(HTMLMailTextPath + "\\Error\\" + Path.GetFileName(filename));
                                File.Move(filename, HTMLMailTextPath + "\\Error\\" + Path.GetFileName(filename));
                            }
                        }
                        else
                        {
                            ...
                            CreateAuditFile(Path.GetFileName(filename), "KGJS_RFQ", VRNO, "Error", "LeS-1001:Unable to find URL in file " + Path.GetFileName(filename), BuyerCode, SupplierCode, AuditPath);
                            if (File.Exists(HTMLMailTextPath + "\\Error\\" + Path.GetFileName(filename))) File.Delete(HTMLMailTextPath + "\\Error\\" + Path.GetFileName(filename));
                            File.Move(filename, HTMLMailTextPath + "\\Error\\" + Path.GetFileName(filename));
                        }
                    }
        ...
        public void DownloadRFQ(string RFQFile)
        {
            ...
            try
            {
                ...
                if (this.DocType == "RFQ")
                {
                    ...
                    if (LoadURL("input", "id", "Details_VendorNotes"))////submitQuery update 05052020
                    {
                        ...
                        if (_isSameRFQ)
                        {
                            ...
                            if (GetHeaderDetails(ref _lesXml, eFile))
                            {
                                if (GetItemDetails(ref _lesXml))
                                {
                                    ...
                                    //}
                                    //else
                                    // Updated By kalpita on 05-05-2020
                                    {
                                    ...
                                    if (GetAddress(ref _lesXml))
                                    {
                                        ...
                                        if (_lesXml.Total_LineItems.Length > 0)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                CreateAuditFile(eFile, "KGJS_RFQ", VRNO, "Error", "LeS-1004:Unable to process file", BuyerCode, SupplierCode, AuditPath);
                                        ...
                                        else
                                        {
                                            ...
                                            CreateAuditFile(eFile, "KGJS_RFQ", VRNO, "Error", "LeS-1004.10:Unable to process file as lineitem count is zero", BuyerCode, SupplierCode, AuditPath);
                                            if (File.Exists(HTMLMailTextPath + "\\Error\\" + Path.GetFileName(RFQFile))) File.Delete(HTMLMailTextPath + "\\Error\\" + Path.GetFileName(RFQFile));
                                            File.Move(RFQFile, HTMLMailTextPath + "\\Error\\" + Path.GetFileName(RFQFile));
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        CreateAuditFile(eFile, "KGJS_RFQ", VRNO, "Error", "LeS-1040:Unable to details - address Field(s) not present.", BuyerCode, SupplierCode, AuditPath);
                                    }
                                }
                                else
                                {
                                    ...
                                    CreateAuditFile(eFile, "KGJS_RFQ", VRNO, "Error", "LeS-1040:Unable to details - RFQ item Field(s) not present.", BuyerCode, SupplierCode, AuditPath);
                                }
                            }
                            else
                            {
                                ...
                                CreateAuditFile(eFile, "KGJS_RFQ", VRNO, "Error", "LeS-1040:Unable to details -RFQ header Field(s) not present.", BuyerCode, SupplierCode, AuditPath);
                            }
                        }
                        else
                        {
                            ...
                            CreateAuditFile(filename, "KGJS_RFQ", VRNO, "Error", "LeS-1004.3:Unable to process file due to VRNo mismatch", BuyerCode, SupplierCode, AuditPath);
                    ...
                    else
                    {
                        ...
                        if (_hTitle != null)
                        {
                            ...
                            CreateAuditFile(Path.GetFileName(RFQFile), "KGJS_RFQ", VRNO, "Error","LeS-1004:Unable to process file due to "+ _hTitle.InnerText.Trim(), BuyerCode, SupplierCode, AuditPath);
                            string filename = PrintScreenPath + "\\KGJS_RFQError_" + DateTime.Now.ToString("ddMMyyyHHmmssfff") + ".png";
                            if (PrintScreen(filename)) filename = "";
                        }
                    }
        ...
        public void WriteErrorLog_With_Screenshot(string AuditMsg, string Filename,string ErrorNo)
        {
            ...
            {
                CreateAuditFile(eFile, "KGJS_HTTP_" + this.DocType, VRNO, "Error", ErrorNo + AuditMsg, BuyerCode, SupplierCode, AuditPath);
            }
        }
                        ...
                        if (GetHeaderDetails(ref _lesXml, eFile))
                        {
                            if (GetItemDetails(ref _lesXml))
                            {
                                if (GetAddress(ref _lesXml))
                                {
                                    ...
                                    if (_lesXml.Total_LineItems.Length > 0)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            CreateAuditFile(eFile, "KGJS_PO", VRNO, "Error", "LeS-1004:Unable to process file", BuyerCode, SupplierCode, AuditPath);
                                            if (File.Exists(HTMLMailTextPath + "\\Error\\" + Path.GetFileName(RFQFile))) File.Delete(HTMLMailTextPath + "\\Error\\" + Path.GetFileName(RFQFile));
                                            File.Move(RFQFile, HTMLMailTextPath + "\\Error\\" + Path.GetFileName(RFQFile));
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        CreateAuditFile(eFile, "KGJS_PO", VRNO, "Error", "LeS-1004.10:Unable to process file as Lineitem count is zero", BuyerCode, SupplierCode, AuditPath);
                                        if (File.Exists(HTMLMailTextPath + "\\Error\\" + Path.GetFileName(RFQFile))) File.Delete(HTMLMailTextPath + "\\Error\\" + Path.GetFileName(RFQFile));
                                        File.Move(RFQFile, HTMLMailTextPath + "\\Error\\" + Path.GetFileName(RFQFile));
                                    }
                                }
                                else
                                {
                                    ...
                                    CreateAuditFile(eFile, "KGJS_PO", VRNO, "Error", "LeS-1040:Unable to get details - address Field(s) not present.", BuyerCode, SupplierCode, AuditPath);
                                }
                            }
                            else
                            {
                                ...
                                CreateAuditFile(eFile, "KGJS_PO", VRNO, "Error", "LeS-1040:Unable to get details - PO item Field(s) not present.", BuyerCode, SupplierCode, AuditPath);
                            }
                        }
                        else
                        {
                            ...
                            CreateAuditFile(eFile, "KGJS_PO", VRNO, "Error", "LeS-1040:Unable to get details - PO header Field(s) not present.", BuyerCode, SupplierCode, AuditPath);
                        }
                    }
                    else
                    {
                        ...
                        CreateAuditFile(filename, "KGJS_PO", VRNO, "Error", "LeS-1004.3:Unable to process file due to VRNo mismatch", BuyerCode, SupplierCode, AuditPath);
                        if (PrintScreen(filename)) filename = "";
                    }
                }
                else
        ...
        private void ProcessQuote(FileInfo file, MTML.GENERATOR.MTMLInterchange interchange)
        {
            ...
            try
            {
                ...
                List<string> slPrice = new List<string>(); slPrice.Clear();
                ...
                if (webDoc != null)
                {
                    if (!webDoc.Body.InnerText.Contains("This Quotation has already been completed") && !webDoc.Body.InnerText.Contains("This Quotation has been Cancelled"))
                    {
                        ...
                        for (int i = 0; i < interchange.DocumentHeader.LineItems.Count; i++)
                        {
                            ...
                            if (item != null && item.IsExtraItem == 0 && !string.IsNullOrEmpty(item.OriginatingSystemRef))
                            {
                                ...
                                else
                                {
                                    ...
                                    slPrice.Add(iCount + "|" + txtUnitPrice.GetAttribute("value"));
                                }
                        ...
                        {
                          //  _browser.Document.InvokeScript("click", new object[] { "this.alert(123)", "JavaScript" });
                           btnCalculate.InvokeMember("click");
                            
                            System.Threading.Thread.Sleep(3000);
                        }

                        foreach (string str in slPrice)
                        {
                        ...
                        if (sendQuote)
                        {
                            ...
                            if (LoadURL("input", "name", "btnChoice"))//added by kalpita on 05-05-20
                            {
                            ...
                            if (PostURL("input", "value", "Close")) // Search Close button on Screen///Close
                            {
                                ...
                                else
                                {
                                    ...
                                    CreateAuditFile(Path.GetFileName(eFile), "KGJS_HTTP_Quote", VRNO, "Error", "LeS-1011:Unable to submit quote for " + VRNO, BuyerCode, SupplierCode, AuditPath);
                            ...
                            else
                            {
                                ...
                                CreateAuditFile(Path.GetFileName(eFile), "KGJS_HTTP_Quote", VRNO, "Error", "LeS-1017:Unable to submit quote as Post URL loading failed for " + VRNO, BuyerCode, SupplierCode, AuditPath);
                                MoveToError(file.FullName);
                            }
                        }
                        else
            ...
            catch (Exception ex)
            {
                ...
                CreateAuditFile(Path.GetFileName(errorScreenShot), "KGJS_HTTP_Quote", VRNO, "Error", "LeS-1004:Unable to process file due to " + ex.Message + ".", BuyerCode, SupplierCode, AuditPath);
